...
BEGIN   
    ...
    IF (vUsrId>0) THEN
        ...
        AND pro.men_id IN (
							SELECT MAX(men_id)
                            FROM men_menu 
                            WHERE usr_id = vUsrId AND ((DATE_FORMAT(NOW(),'%H')*60)+DATE_FORMAT(NOW(),'%i')) BETWEEN men_time_start 
								AND men_time_end
                                AND men_enable = 1
						  )
        ORDER BY
...
-- Dump completed on 2020-06-17 19:34:42
